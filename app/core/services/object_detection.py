...
def _build_providers(model_path: str, model_key: str):
    ...
    if "DmlExecutionProvider" in available:
        ...
        if "AzureExecutionProvider" in available:
            ...
            print(
                "[ONNXRuntime] This usually means 'onnxruntime' CPU package is overriding 'onnxruntime-directml'."
            )
            print(
                "[ONNXRuntime] Fix: pip uninstall onnxruntime && pip install --force-reinstall onnxruntime-directml"
            )
...
def _create_inference_session(
    ...
    try:
        ...
        if (
            ...
            and providers[0][0] == "DmlExecutionProvider"
        ):
            print(
                "[ONNXRuntime] Retrying DirectML init with default provider options..."
            )
            return ort.InferenceSession(
                model_path,
...
def _warmup_session(session: Any, model_key: str):
    ...
    print(
        f"[WARMUP] Running 3 warmup inferences for '{model_key}' (DirectML shader compile)..."
    )
    for i in range(3):
        t0 = time.time()
    ...
    if model_key not in ONNX_SESSIONS:
        ...
        for compatibility_mode in init_profiles:
            ...
            try:
                ...
                if use_gpu and not compatibility_mode:
                    print(
                        "[ONNXRuntime] Optimized DirectML init failed. Retrying compatibility profile..."
                    )
                else:
                    dml_mode = use_gpu
                    if dml_mode:
