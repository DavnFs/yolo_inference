...
BEV_DESTINATION_POINTS = np.float32(
    [[0, BEV_HEIGHT], [0, 0], [BEV_WIDTH, 0], [BEV_WIDTH, BEV_HEIGHT]]
)
PIXELS_PER_METER = (
    8.0  # Menyesuaikan rasio kedalaman yang lebih realistis (1 meter = 8 piksel BEV)
)
STEREO_FX = 721.53  # DrivingStereo / KITTI focal length (pixels, 1242px original)
HAZARD_COLORS = {
    "danger": (0, 0, 255),
    "warning": (0, 255, 255),
    "safe": (0, 255, 0),
    "out_of_roi": (128, 128, 128),
}
...
def discover_models(models_dir: str) -> Dict:
    ...
    onnx_files = [
        ...
        if f.lower().endswith(".onnx") and ".ort_optimized" not in f.lower()
    ]
    ...
    for filename in sorted(onnx_files):
        ...
        if is_faster_rcnn:
            configs[model_key] = {"path": full_path, "type": "FASTER-RCNN"}
        else:
            configs[model_key] = {
                "path": full_path,
                "type": "YOLO",
        ...
        print(
            f"[AUTO-DISCOVER] Found model: '{model_key}' | type: {'FASTER-RCNN' if is_faster_rcnn else 'YOLO'} | {filename}"
        )
...
class GPUSession:
    """Wrap an ONNX session with IO binding to minimize CPU<->GPU copies."""

    def __init__(self, session: ort.InferenceSession, device_id: int = 0):
        self.session = session
        active_ep = session.get_providers()[0]
        self.device_type = _GPU_DEVICE_TYPES.get(active_ep, "cuda")
    ...
    def run(
        self, feed_dict: Dict[str, np.ndarray], output_names: List[str] | None = None
    ):
        ...
        for name, value in feed_dict.items():
            ...
            ort_value = ort.OrtValue.ortvalue_from_numpy(
                input_np, self.device_type, self.device_id
            )
...
def _build_session_options(
    use_gpu: bool, compatibility_mode: bool = False
) -> ort.SessionOptions:
    ...
    if use_gpu:
        ...
        # Let GPU scheduling dominate; avoid CPU thread contention.
        sess_options.intra_op_num_threads = 1
        sess_options.inter_op_num_threads = 1

        # Izinkan CPU EP sebagai fallback untuk node yang tidak bisa di-TRT
        # (misal INT64 ops). Tanpa ini TRT init gagal berkali-kali → 20 menit compile.
        try:
        ...
        # Use most physical cores but keep inter-op single-threaded
        cpu_count = os.cpu_count() or 6
        sess_options.intra_op_num_threads = max(1, min(cpu_count, 6))
        sess_options.inter_op_num_threads = 1
...
def _build_providers(model_path: str, model_key: str):
    ...
    gpu_providers = [
        (
            "TensorrtExecutionProvider",
            {
                ...
                # Izinkan node yang tidak kompatibel TRT (INT64, dll) jalan di CUDA/CPU
                # tanpa ini model YOLO11 sering gagal init karena ada INT64 nodes
    ...
    for ep_tuple in gpu_providers:
        ep_name = ep_tuple[0]
        if ep_name in available:
            use_gpu = True
            providers.append(ep_tuple)
            print(f"[ONNXRuntime] Using {ep_name} (GPU mode).")
            break

    if not use_gpu:
        print("[ONNXRuntime] No GPU provider available. Using CPU only.")
        print("[ONNXRuntime] For GPU acceleration install: pip install onnxruntime-gpu")
...
def _build_optimized_model_path(model_path: str) -> str:
    ...
    if root.lower().endswith(".ort_optimized"):
        return model_path
    return f"{root}.ort_optimized{ext}"


def _create_inference_session(
    model_path: str, sess_options: ort.SessionOptions, providers: list
):
    try:
...
def _run_session(
    session: Any, output_names: List[str] | None, inputs: Dict[str, np.ndarray]
):
    if isinstance(session, GPUSession):
...
def _warmup_session(session: Any, model_key: str):
    ...
    print(
        f"[WARMUP] Running 3 warmup inferences for '{model_key}' (GPU kernel compile)..."
    )
    for i in range(3):
        t0 = time.time()
...
def load_onnx_model_once(model_path: str, model_key: str):
    ...
    if model_key not in ONNX_SESSIONS:
        ...
        optimized_model_path = (
            _build_optimized_model_path(model_path) if use_gpu else None
        )
        last_error = None

        for compatibility_mode in init_profiles:
            sess_options = _build_session_options(
                use_gpu=use_gpu, compatibility_mode=compatibility_mode
            )
            ...
            if use_gpu and not compatibility_mode:
                if optimized_model_path and os.path.exists(optimized_model_path):
                    ...
                    print(
                        f"[ONNXRuntime] Using cached optimized model: {optimized_model_path}"
                    )
                else:
                    try:
                        ...
                        print(
                            f"[ONNXRuntime] Warning: cannot set optimized model output: {e}"
                        )
            ...
            try:
                ...
                if use_gpu and not compatibility_mode:
                    print(
                        "[ONNXRuntime] Optimized GPU init failed. Retrying compatibility profile..."
                    )
                else:
                    gpu_mode = use_gpu
                    if gpu_mode:
                        err_msg = (
                            f"GPU gagal inisialisasi untuk model '{model_key}'. "
                            "Model ini kemungkinan tidak kompatibel dengan GPU provider pada kombinasi driver/CUDA/ONNX Runtime saat ini. "
                            "Coba model ONNX lain yang kompatibel atau export ulang model dengan graph yang lebih sederhana."
                        )
                    else:
        ...
        print(
            f"ONNX session for '{model_key}' loaded. Provider: {ONNX_SESSIONS[model_key].get_providers()}"
        )
...
def get_faster_rcnn_prediction_from_frame(
    image_bgr: np.ndarray, model_path: str
) -> Dict:
    ...
    inputs = {"detection_input": detection_input_np, "weather_input": weather_input_np}
    ...
    for i, score in enumerate(onnx_scores):
        if score > 0.5:
            ...
            label = (
                FASTER_RCNN_CLASSES[class_id]
                if class_id < len(FASTER_RCNN_CLASSES)
    ...
    if weather_logits.ndim == 1:
        ...
        weather_idx = int(
            np.bincount(np.argmax(weather_logits, axis=0).ravel()).argmax()
        )
...
def get_faster_rcnn_prediction(image_bytes: bytes, model_path: str) -> Dict:
    return get_faster_rcnn_prediction_from_frame(
        _decode_image_bytes(image_bytes), model_path
    )
...
def postprocess_yolo(output, scale, dw, dh, labels, orig_w: int, orig_h: int):
    ...
    indices = cv2.dnn.NMSBoxes(
        boxes_nms.tolist(), scores.tolist(), YOLO_CONF_THRESH, YOLO_NMS_THRESH
    )
    final_detections = []
    if len(indices) > 0:
...
def get_yolo_labels(session, labels_path: str, model_key: str):
    ...
    sess = session.session if isinstance(session, GPUSession) else session
    try:
        ...
        if "names" in meta_dict:
            ...
            print(
                f"[LABELS] Auto-extracted {len(labels)} classes from ONNX metadata for {model_key}"
            )
            return labels
    except Exception as e:
...
def get_yolo_prediction_from_frame(
    image_bgr: np.ndarray, model_path: str, labels_path: str
) -> Dict:
...
def get_yolo_prediction(image_bytes: bytes, model_path: str, labels_path: str) -> Dict:
    return get_yolo_prediction_from_frame(
        _decode_image_bytes(image_bytes), model_path, labels_path
    )
...
def _run_path_planning(
    ...
    try:
        ...
        abs_poly = (
            ROAD_POLYGON_POINTS_RELATIVE * np.array([w, h], dtype=np.float32)
        ).astype(np.int32)

        if any("hazard_level" not in det for det in results.get("detections", [])):
            analyzed_dets, _ = perform_road_analysis(
                (h, w), results.get("detections", [])
            )
            results["detections"] = analyzed_dets

        # Override homography distance with true stereo depth
        for det in results.get("detections", []):
            x1, y1, x2, y2 = det.get("bounding_box", [0, 0, 0, 0])

            # Mencegah error out-of-bounds array saat sampling
            x1, y1 = max(0, int(x1)), max(0, int(y1))
            x2, y2 = min(w, int(x2)), min(h, int(y2))

            # --- ROBUST PERCENTILE DEPTH EXTRACTION ---
            patch = external_depth[y1:y2, x1:x2]
            ...
            if 0.5 < true_dist < 100.0:
                ...
                # --- UPDATE WARNA KOTAK UI SESUAI JARAK FISIK ---
                if true_dist <= 15.0:
        ...
        print(f"[PATH PLANNING CRASH] {traceback.format_exc()}")
        results["path_planning"] = {
            "path_found": False,
            "waypoints": [],
            "error": str(e),
            "obstacle_grid": np.zeros((400, 200), dtype=np.uint8),
        }
    return results
...
def get_combined_prediction_from_frame(
    ...
    if not config:
        return {
            "error": f"Konfigurasi untuk model '{active_model_key}' tidak ditemukan."
        }
    ...
    try:
        if model_type == "FASTER-RCNN":
            ...
            results = get_yolo_prediction_from_frame(
                frame_bgr, model_path, config["labels_path"]
            )
        else:
...
def _get_frame_geometry(
    ...
    if cache_key not in GEOMETRY_CACHE:
        ...
        perspective_matrix = cv2.getPerspectiveTransform(
            abs_polygon_points, BEV_DESTINATION_POINTS
        )
...
def perform_road_analysis(
    frame_shape: tuple, detections: list
    ...
    for det in detections:
        ...
        try:
            intersection_area, _ = cv2.intersectConvexConvex(
                abs_polygon_points, bbox_points
            )
        except cv2.error:
            intersection_area = 0
