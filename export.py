"""
Download Depth Anything V2 Small ONNX and verify it with ONNX Runtime DirectML.
...
Requirements:
    pip install huggingface_hub onnx onnxruntime-directml
...
def _select_providers():
    ...
    print("      WARNING: DmlExecutionProvider is not available. Falling back to CPU.")
    return ["CPUExecutionProvider"]


def verify_directml(model_path: Path):
    ...
    if "DmlExecutionProvider" not in active_providers:
        raise RuntimeError("DirectML provider is not active for this ONNX model.")

    print("      DirectML verification PASSED.")


def main():
    model_path = download_onnx_model()
    validate_onnx_model(model_path)
    verify_directml(model_path)
